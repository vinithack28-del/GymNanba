<x-layouts.admin
    title="Settings"
    eyebrow="{{ __('settings.eyebrow') }}"
    heading="{{ __('settings.heading') }}"
    subheading="{{ __('settings.subheading') }}"
>
    <div class="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <section class="space-y-6">
            ...
            <article class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                ...
                <h3 class="mt-2 text-2xl font-semibold">{{ $activeSessions }} active session</h3>
        ...
        <section class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
            ...
            <div class="mt-6 space-y-4">
                @foreach ($languages as $language)
                    <form method="POST" action="{{ route('admin.settings.languages.update', $language) }}" class="rounded-[1.5rem] border border-white/10 bg-slate-950/50 p-4">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div>
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="rounded-full {{ $language->is_active ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-500/15 text-slate-300' }} px-3 py-1 text-xs uppercase tracking-[0.2em]">
                            ...
                            <button
                                type="submit"
                                class="rounded-full {{ $language->is_active ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-orange-500 text-slate-950 hover:bg-orange-400' }} px-4 py-2 text-sm font-semibold transition"
                            >
                        ...
                        <div class="mt-4 h-2 rounded-full bg-white/10">
                            <div class="h-2 rounded-full {{ $language->completeness_pct >= 90 ? 'bg-emerald-400' : 'bg-amber-400' }}" style="width: {{ min(100, $language->completeness_pct) }}%"></div>
                        </div>
                    </form>
                @endforeach
            </div>
