...
const form = useForm({
    ...
    price_paise: props.plan?.price_paise || '',
    gst_applicable: props.plan?.gst_applicable || false,
    gst_rate: props.plan?.gst_rate || props.defaultGstRate || 18,
    branch_ids: props.plan?.branches?.map(b => b.id) || [],
    status: props.plan?.status || 'active',
});

const submit = () => {
    if (isEdit) {
        form.put(`/tenant/plans/${props.plan.id}`);
    } else {
        form.post('/tenant/plans');
    }
};
</script>
    ...
    <AppLayout>
        ...
        <div class="flex flex-col gap-5">
            <div class="flex items-start justify-between">
                ...
                <Link href="/tenant/plans" class="flex items-center gap-2 rounded-full border border-white/10 bg-slate-950/50 px-4 py-2.5 text-sm font-medium text-slate-300 hover:bg-white/5">
            ...
            <form @submit.prevent="submit" class="rounded-2xl border border-white/10 bg-white/5 p-6">
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input v-model="form.price_paise" type="number" step="0.01" placeholder="e.g. 1000" class="w-full rounded-lg border border-white/10 bg-slate-950/70 px-3 py-2 text-sm text-slate-300 outline-none focus:border-orange-400" required>
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <label v-for="branch in branches" :key="branch.id" class="flex items-center gap-2 rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2">
