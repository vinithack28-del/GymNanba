        ...
        <div class="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
            ...
            <section class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                <div class="flex items-center justify-between">
                    ...
                    <span class="rounded-full bg-slate-950/70 px-3 py-1 text-xs text-slate-300">Enable only if completeness Ã¢â€°Â¥ 90%</span>
                </div>
                    ...
                    <form v-for="language in languages" :key="language.id" @submit.prevent="toggleLanguage(language)" class="rounded-[1.5rem] border border-white/10 bg-slate-950/50 p-4">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div>
                                ...
                                <p class="mt-2 text-sm text-slate-400">{{ language.locale_code }} Ã‚- Completeness {{ language.completeness_pct }}%</p>
                            </div>
