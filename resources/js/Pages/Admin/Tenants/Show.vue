        ...
        <div class="flex flex-col gap-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-3xl font-semibold">{{ tenant.gym_name }}</h1>
                    <p class="mt-1 text-slate-300">Profile overview, active subscriptions, and recorded payments.</p>
                </div>
                <div class="flex items-center gap-2">
                    <Link
                        href="/admin/tenants"
                        class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-medium transition hover:bg-white/10"
                    >
            ...
            <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 p-6 sm:p-8">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div class="flex items-center gap-4">
                        <div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-orange-500/20 text-2xl font-bold text-orange-300">
                            {{ tenant.gym_name.charAt(0).toUpperCase() }}
                        </div>
                        <div>
                            <h2 class="text-2xl font-bold">{{ tenant.gym_name }}</h2>
                            <p class="mt-1 text-sm text-slate-400">{{ tenant.business_type }} · {{ tenant.city }}, {{ tenant.state }}</p>
                            <p class="mt-1 font-mono text-xs text-slate-500">{{ tenant.primary_domain }}</p>
                        </div>
                    </div>
                    <span class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold ring-1" :class="[getStatusColors(tenant.status).bg, getStatusColors(tenant.status).text, getStatusColors(tenant.status).ring]">
                        <span class="h-2 w-2 rounded-full" :class="getStatusColors(tenant.status).dot"></span>
                        {{ tenant.status.charAt(0).toUpperCase() + tenant.status.slice(1) }}
                    </span>
                </div>

                <div class="mt-6 grid grid-cols-2 gap-3 border-t border-white/10 pt-6 sm:grid-cols-4">
                    <div>
                        <p class="text-xs uppercase tracking-[0.22em] text-slate-400">Members</p>
                        <p class="mt-1 text-2xl font-bold">{{ tenant.members_count }}</p>
                    </div>
                    <div>
            ...
            <div class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                <div class="space-y-6">
                    ...
                    <section class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                        <h3 class="mb-5 text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">Routing & Technical</h3>
                        <div class="grid grid-cols-2 gap-0">
                            <div class="border-b border-r border-white/10 p-4">
                            ...
                            <div v-if="tenant.domain_mode === 'separate' && tenant.custom_domain" class="col-span-2 p-4">
                    ...
                    <section v-if="tenant.notes" class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                        <h3 class="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">Internal Notes</h3>
                ...
                <div class="space-y-6">
                    <section class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                        <div class="mb-5 flex items-center justify-between">
                            <h3 class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">Subscriptions</h3>
                        ...
                        <div class="space-y-3">
                            <div v-if="tenant.subscriptions && tenant.subscriptions.length > 0" v-for="subscription in tenant.subscriptions.sort((a, b) => b.id - a.id)" :key="subscription.id" class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            ...
                            <div v-else class="flex flex-col items-center gap-2 py-8 text-center">
                                <svg class="h-8 w-8 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>
                    ...
                    <section class="rounded-[2rem] border border-white/10 bg-white/5 p-6">
                        <div class="mb-5 flex items-center justify-between">
                            <h3 class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">Payments</h3>
                        ...
                        <div class="space-y-3">
                            <div v-if="tenant.payments && tenant.payments.length > 0" v-for="payment in tenant.payments.sort((a, b) => new Date(b.paid_at) - new Date(a.paid_at))" :key="payment.id" class="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <div class="flex items-start justify-between gap-3">
                            ...
                            <div v-else class="flex flex-col items-center gap-2 py-8 text-center">
                                <svg class="h-8 w-8 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
